...
export default class ScanQR extends React.Component<ParentProps, State> {
	...
	storeUserCredsInContext = (name: string, sovrinDid: ISovrinDid) => {
		let userDid: ISovrinDid = {
			name: name,
			did: sovrinDid.did,
			verifyKey: sovrinDid.verifyKey,
			encryptionPublicKey: sovrinDid.encryptionPublicKey,
			secret: sovrinDid.secret
		};

		return (
			<Consumer>
				{({ setUser }: { setUser: Function }) => {
					setUser(userDid);
				}}
			</Consumer>
		);
	...
	handleUnlock = () => {
		if (this.state.payload && this.state.password) {
			try {
				...
				debugger;
				this.storeUserCredsInContext(mnemonicJson.name, generateSovrinDID(mnemonicJson.mnemonic));
			...
			<View style={ModalStyle.modalOuterContainer}>
				<View style={ModalStyle.modalInnerContainer}>
					...
					<View style={{ flexDirection: 'row', justifyContent: 'center', paddingBottom: 2 }}>
						<Button onPress={() => this.handleResetErrors()} bordered dark style={{ width: '100%', justifyContent: 'center' }}>
							<Text>RETRY PASSWORD</Text>
						</Button>
					</View>
					<View style={{ flexDirection: 'row', justifyContent: 'center', paddingBottom: 10 }}>
						<Button onPress={() => this.handleResetScan()} bordered dark style={{ width: '100%', justifyContent: 'center' }}>
