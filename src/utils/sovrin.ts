...
export function getSignature(payload: object): Promise<any> {
  return new Promise((resolve, reject) => {
    // @ts-ignore
    SInfo.getItem(SecureStorageKeys.encryptedMnemonic, {}).then(
      (encryptedMnemonic: string | null) => {
        if (encryptedMnemonic) {
          // @ts-ignore
          SInfo.getItem(SecureStorageKeys.password, {}).then(
            (password: string | null) => {
              if (password) {
                const mnemonicJson: IMnemonic = Decrypt(
                  encryptedMnemonic,
                  password,
                );
                const sovrinDid: ISovrinDid = generateSovrinDID(
                  mnemonicJson.mnemonic,
                );
                const payloadSig = sovrin.signMessage(
                  JSON.stringify(payload),
                  sovrinDid.secret.signKey,
                  sovrinDid.verifyKey,
                );
